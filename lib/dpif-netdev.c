...
struct sched_numa {
    ...
    /* PMDs on numa node. */
    struct sched_pmd *pmds;//在此numa中的pmd引用（数组）
    /* Num of PMDs on numa node. */
    unsigned n_pmds;//有多少个pmd（数组长度）
...
pmd_thread_main(void *f_)
{
    ...
    //为了保证dpdk收发包，设置dpdk需要的core信息
    dpdk_attached = dpdk_attach_thread(pmd->core_id);
    //线程创建后已加入我们需要负责哪些port
