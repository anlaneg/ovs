            ...
            enum json_type value_type, const struct json **value)
{
    if (json->type != JSON_ARRAY//json不是数组类型
        || json->array.n != 2//成员数不为2
        || json->array.elems[0]->type != JSON_STRING//成员0不是字符串
        || (name && strcmp(json->array.elems[0]->string, name))//有name，但与name不相同
        || json->array.elems[1]->type != value_type)//成员1类型与value类型不一致
    {
        *value = NULL;
                        ...
                        struct ovsdb_symbol_table *symtab)
{
    ...
    if (ovsdb_type_is_map(type)
        || (json->type == JSON_ARRAY
            && json->array.n > 0//多个成员
            && json->array.elems[0]->type == JSON_STRING//成员类型为字符串
            && !strcmp(json->array.elems[0]->string, "set"))) {//成员0的值为'set'
