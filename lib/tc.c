...
calc_offsets(struct tc_action *action, struct flower_key_to_pedit *m,
             int *cur_offset/*出参，记录起始offset*/, int *cnt/*出参，总长度占sizeof(int)的总数*/, ovs_be32 *last_word_mask/*出参，为size对齐，而在右侧需要对齐的mask*/,
             ovs_be32 *first_word_mask/*出参，为start对齐，而在左侧需要对齐的mask*/, ovs_be32 **mask/*出参，mask指针*/, ovs_be32 **data/*出参，data指针*/)
{
                                 ...
                                 struct tc_action *action)
{
    ...
    for (i = 0; i < ARRAY_SIZE(flower_pedit_map); i++) {
        ...
        calc_offsets(action, m, &cur_offset/*字段起始offset*/, &cnt/*字段总长度,单位sizeof(int)=4*/, &last_word_mask,
