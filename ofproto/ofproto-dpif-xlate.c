    ...
    OVS_REQ_RDLOCK(ms->rwlock)
{
    ...
    LIST_FOR_EACH(mrouter, mrouter_node, &ms->mrouter_lru) {//遍历我们所有的路由侧端口
        ...
        if (mcast_xbundle && mcast_xbundle != in_xbundle
            ...
            output_normal(ctx, mcast_xbundle, xvlan);//向路由侧端口前进
        } else if (!mcast_xbundle) {
            xlate_report(ctx, OFT_WARN,
                ...
                bool is_last_action)
{
    ...
    LIST_FOR_EACH (bucket, list_node, &group->up.buckets) {
        ...
        xlate_group_bucket(ctx, bucket, last);//完成转换
    }
                     ...
                     bool is_last_action)
{
    ...
    case OFPGT11_INDIRECT:
        //新针对flow做一次分支转换（相当于将这个包做个快照，改完，处理了，再回退回来）
                    ...
                    bool is_last_action)
{
    ...
    if (bundle->dst.field) {
        ...
        xlate_output_action(ctx, port, 0, false, is_last_action);//从选出的口扔出去（在选出的口进行转换）
    }
}
