...
struct xbundle {
    ...
    unsigned long *trunks;         /* Bitmap of trunked VLANs, if 'vlan' == -1.
                                    * NULL if all VLANs are trunked. *///trunk口对应的vlans bitmap
...
struct xlate_ctx {
    ...
    //处理stack的push,pop action完成内存压弹栈
    struct ofpbuf stack;
    ...
    OVS_REQ_RDLOCK(ms->rwlock)
{
    ...
    LIST_FOR_EACH(mrouter, mrouter_node, &ms->mrouter_lru) {//遍历我们所有的路由侧端口
        ...
        if (mcast_xbundle && mcast_xbundle != in_xbundle
            ...
            mcast_output_add(out, mcast_xbundle);//向路由侧端口前进
        } else if (!mcast_xbundle) {
            xlate_report(ctx, OFT_WARN,
                         "mcast router port is unknown, dropping");//还没有学习到路由侧端口，丢包
        } else if (mrouter->vlan != xvlan->v[0].vid) {
            xlate_report(ctx, OFT_DETAIL,
                         "mcast router is on another vlan, dropping");//现在已学习到的路由侧端口，不收取此vlan
        } else {
            xlate_report(ctx, OFT_DETAIL,
                         "mcast router port is input port, dropping");//对我们而言是同一接口，路由器应已经知道了，我们丢包即可
        }
    }
}
    ...
    if (mcast_snooping_enabled(ctx->xbridge->ms)//开启了igmp snooping
        ...
        if (mac_port) {
            ...
            if (mac_xbundle
                ...
                output_normal(ctx, mac_xbundle, &xvlan);//发送给fdb学习到的端口
            } else if (!mac_xbundle) {
                xlate_report(ctx, OFT_WARN,
                     ...
                     bool truncate)
{
    ...
    tnl_push_data.tnl_port = tunnel_odp_port;//这个应与xport->ofport相同，需要查看上层调用（上层保证了这一点)
                         ...
                         bool is_last_action)
{
    ...
    if (ofc->zone_src.field) {
        /*如果指定的zone_src，则自flow中提取zone*/
        zone = mf_get_subfield(&ofc->zone_src, &ctx->xin->flow);
    } else {
        /*如果未指定zonee_src，则自zone立即数中提取*/
        zone = ofc->zone_imm;
    }
                 ...
                 bool group_bucket_action/*当前是否在转换group_bcket的action*/)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        if (ctx->exit) {
             ...
             * execution. */
            if (ctx->freezing) {
                freeze_unroll_actions(a/*action当前位置*/, ofpact_end(ofpacts, ofpacts_len)/*action结尾*/,
                                      ctx);
            }
            break;
        }
        ...
        //按action类型，进行flow处理及action生成，以下action将触发action生成
        //output,controller,enqueue,resubmit
        //以下action将触发递归action生成
        //group,
        //以下action将触发flow设置
        //set_vlan_vid,set_vlan_pcp，strip_vlan，push_vlan,set_eth_src
        //set_eth_dst,set_ipv4_src,set_ipv4_dst,set_ip_dscp,set_ip_ecn,
        //set_ip_ttl,set_l4_src_port,set_l4_dst_port,set_tunnel
        switch (a->type) {
        case OFPACT_OUTPUT:
        ...
        case OFPACT_CONTROLLER:
            ...
            if (controller->pause) {
                ...
                xlate_controller_action(ctx, controller->max_len,//将报文传递
                                        controller->reason,
        ...
        case OFPACT_SET_VLAN_VID:
            ...
            if (flow->vlans[0].tci & htons(VLAN_CFI) ||
                ...
                if (!flow->vlans[0].tpid) {
                	//之前没有vlan,指明首个vlan的eth_type_vlan
                    flow->vlans[0].tpid = htons(ETH_TYPE_VLAN);
                }
                //丢弃掉原有的vlan位，存入新的vlan及cfi
        ...
        case OFPACT_SET_L4_SRC_PORT:
        	//如果是ipv4或ipv6报文，并且非首片报文，则设置传输层src port,同时设置mask设置源port
        ...
        case OFPACT_SET_L4_DST_PORT:
        	//如果是ipv4或ipv6报文，并且非首片报文，则设置目的port,同时设置mask设置源port
            if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
                memset(&wc->masks.nw_proto, 0xff, sizeof wc->masks.nw_proto);
        ...
        case OFPACT_OUTPUT_TRUNC:
            //等同与output,仅去向controller时，max_len生效。
    ...
    *xout = (struct xlate_out) {
    ...
    if (xin->frozen_state) {
        ...
        if (state->action_set_len) {
            /*recirc中有action_set，需要先将action_set中的动作执行完成。*/
    ...
    if (!xin->frozen_state && process_special(&ctx, in_port)) {
        ...
        if (!ecn_drop
            ...
            if (xin->ofpacts) {
            	...
            	//未指定action,但已经命中了规则,取出对应actions
                const struct rule_actions *actions
            ...
            //执行ofctl rule action转换（完成在ctx中的动作合并）
