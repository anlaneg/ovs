...
struct ipam_info {
    ...
    unsigned long *allocated_ipv4s; /* A bitmap of allocated IPv4s */ //一个bitmap结构，用于进行ip地址申请释放
    bool ipv6_prefix_set;//是否设置了ipv6前缀
    struct in6_addr ipv6_prefix;//ipv6前缀信息
};
...
struct ovn_datapath {
    ...
    /* Logical switch data. */
    struct ovn_port **router_ports;//此datapath中有多少个与路由器相连的接口（放交换机接口）
    size_t n_router_ports;

    struct hmap port_tnlids;//分配port-id的hash表
    uint32_t port_key_hint;//上一次分配的id号

    bool has_unknown;

    /* IPAM data. */
    struct ipam_info *ipam_info;//此网段的ipadm_info（用于进行ip分配）
...
ovn_datapath_update_external_ids(struct ovn_datapath *od)
{
    ...
    smap_add(&ids, key, uuid_s);//存入此row对应的uuid（即虚设备id号）
    smap_add(&ids, "name", name);//存放虚设备名称
    if (name2 && name2[0]) {
        smap_add(&ids, "name2", name2);//存入虚设备name2,如"neutron:network_name"规定的名称
    }
               ...
               struct ovs_list *both)
{
    ...
    //遍历北向库中的switch,如果此siwtch在datapaths中存在，则其不应为sb_only,而应为both(南北向均存在）
    //故会自sb_only上移除，在both上添加。
    //另外：如果北向库的switch在datapath中不存在，则为nb_only,这个datapath需要同步到南向库
    const struct nbrec_logical_switch *nbs;
    NBREC_LOGICAL_SWITCH_FOR_EACH (nbs, ctx->ovnnb_idl) {
        ...
        init_ipam_info_for_datapath(od);//初始化datapath的ipam
    }

    //遍历北向库的router,如果此router在datapath中存在，则其不应为sb_only,而应为both
    //另外：如果北向库的router在datapath中不存在，则为nb_only,这个datapath需要同步到南向库
    const struct nbrec_logical_router *nbr;
...
build_datapaths(struct northd_context *ctx, struct hmap *datapaths)
{
    ...
    if (!ovs_list_is_empty(&nb_only)) {
        ...
        LIST_FOR_EACH (od, list, &nb_only) {
            ...
            //在sb库中插入记录(datapath_binding记录），并更新tunnel_key
    ...
    LIST_FOR_EACH_SAFE (od, next, list, &sb_only) {
        ...
        ovn_datapath_destroy(datapaths, od);//sb_only的datapath将自datapaths中移除
    }
}

struct ovn_port {
    ...
    //port上ip,mac信息（如果对端是路由器，则用路由器接口信息）
    struct lport_addresses *lsp_addrs;  /* Logical switch port addresses. */
    unsigned int n_lsp_addrs;

    //port security地址组，每一项包含一个mac地址与多个ip
    struct lport_addresses *ps_addrs;   /* Port security addresses. */
    unsigned int n_ps_addrs;

    /* Logical router port data. */
    const struct nbrec_logical_router_port *nbrp; /* May be NULL. */ //router的port配置信息

    struct lport_addresses lrp_networks;//此port的地址信息(网段，ip，mac等）

    bool derived; /* Indicates whether this is an additional port
                   * derived from nbsp or nbrp. */ //指明这是一个附件的口
    ...
    struct ovn_datapath *od;//此port所属的datapath
                            ...
                            const struct nbrec_logical_switch_port *nbsp)
{
    ...
    bitmap_set1(tag_alloc_node->allocated_tags, *nbsp->tag);//占用parent_name对应的vlan位
}
    ...
    const struct nbrec_logical_switch_port *nbsp)
{
    ...
    //给出了mac地址，检查发现mac地址与现有记录不相等，更新南库配置
                   ...
                   struct ovs_list *nb_only, struct ovs_list *both)
{
    ...
    const struct sbrec_port_binding *sb;
    //加载南向库中已有的port信息(优先认为它是sb_only的）
    ...
    HMAP_FOR_EACH (od, key_node, datapaths) {
        if (od->nbs) {
        	//处理datapath是交换机的情况。针对此datapath上的每个port
        	//查询port表，如果port表中存在，则此port在南北向库中均存在
        	//记为both,如果不存在，则此port仅在nb_only上存在，需要同步到南向库。
            for (size_t i = 0; i < od->nbs->n_ports; i++) {
                const struct nbrec_logical_switch_port *nbsp
                    = od->nbs->ports[i];
            ...
            for (size_t i = 0; i < od->nbr->n_ports; i++) {
                ...
                if (op) {
                	//ports中已有这个port了，说明南北向库均已包含（置为both)
                    if (op->nbsp || op->nbrp) {
                        static struct vlog_rate_limit rl
                            = VLOG_RATE_LIMIT_INIT(5, 1);
                ...
                if (redirect_chassis || op->nbrp->n_gateway_chassis) {
                    ...
                    //构造重定向接口名称，构造重定向接口 cr-%s
                    char *redirect_name = chassis_redirect_name(nbrp->name);
                    struct ovn_port *crp = ovn_port_find(ports, redirect_name);
                    if (crp) {
                        crp->derived = true;//标记此口是内部生成的
                        crp->nbrp = nbrp;//使用与原nbrp一样的配置
                        ovs_list_remove(&crp->list);
                        ovs_list_push_back(both, &crp->list);
                    } else {
                        crp = ovn_port_create(ports, redirect_name,
                                              NULL, nbrp, NULL);//人为增加一个port
                        crp->derived = true;
                        ovs_list_push_back(nb_only, &crp->list);//定为nb仅有
                    }
                    crp->od = od;
                    free(redirect_name);
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        if (op->nbsp && !strcmp(op->nbsp->type, "router") && !op->derived) {
            ...
            if (!peer_name) {
            	//接口指明是'router'但没有指定'rotuer-port'，属错误配置，忽略
                continue;
            }

        	//即此处仅处理交换机上与路由器相连接接口
            //查找这个peer,这个peer一定是一个路由器接口
            struct ovn_port *peer = ovn_port_find(ports, peer_name);
            if (!peer || !peer->nbrp) {
                continue;
            }

            peer->peer = op;//使路由器接口标记op
            op->peer = peer;//op也指向此路由器接口
            op->od->router_ports = xrealloc(
                op->od->router_ports,
                sizeof *op->od->router_ports * (op->od->n_router_ports + 1));
            op->od->router_ports[op->od->n_router_ports++] = op;//存放交换机接口
        	...
        	//处理路由器上有peer的接口（要求非人为生成接口）
            struct ovn_port *peer = ovn_port_find(ports, op->nbrp->peer);
            if (peer) {
                if (peer->nbrp) {
                    op->peer = peer;//对端为路由器，更新对端信息
                } else if (peer->nbsp) {
                	//这种情况应在此个顶层if中已处理过，故这里报错
                      ...
                      struct hmap *chassis_qdisc_queues)
{
    ...
    if (op->nbrp) {
    	...
    	//路由器接口对外有三种type:chassisredirect,l3gateway,path
        const char *chassis = smap_get(&op->od->nbr->options, "chassis");
        if (op->derived) {
        	//如果此port是人为添加的，则将其类型更改为chassisredirect
            sbrec_port_binding_set_type(op->sb, "chassisredirect");
        } else if (chassis) {
            sbrec_port_binding_set_type(op->sb, "l3gateway");
        } else {
        	//如果路由器选项中没有'chassis‘配置，则接口类型为patch口
            ...
            const struct chassis_index *chassis_index, struct hmap *ports)
{
    ...
    //区分哪些port共有，哪些port sb有，哪些port nb有
...
static const struct multicast_group mc_unknown = { MC_UNKNOWN, 65534 };//fdb表miss组播组
...
struct ovn_lflow {
    ...
    struct ovn_datapath *od;//属于哪个datapath
    enum ovn_stage stage;//流所处理pipeline位置
    uint16_t priority;//流优先级
    char *match;
    char *actions;
    char *stage_hint;
    const char *where;//debug信息那处生成的规则
};
                 ...
                 const char *stage_hint, const char *where)
{
    ...
    struct ovn_lflow *lflow = xmalloc(sizeof *lflow);//申请flow内存
 ...
 */
//port security的arp协议上的处理（如果指定mac，则mac不能dummy,如果有ip，则ip不能dummy)
static void
build_port_security_nd(struct ovn_port *op, struct hmap *lflows)
{
    ...
    //除容许的arp,nd协议报文外，其它均认为是不容许的，丢掉此包，防止伪造。
    ds_clear(&match);
    ...
    for (size_t i = 0; i < op->n_ps_addrs; i++) {
        ...
        if (ps->n_ipv4_addrs) {
            ...
            if (pipeline == P_IN) {
            	...
            	//由于启用了port security,已限制了源mac发送，现在还要限制src-ip发送，但在限制前
            	//需要放通dhcp 无地址时的交互报文，使其可以通过
...
//构造options_action,response_action及ipv4_addr_match
//其中options_action 是dhcp报文中需要包含的选项信息
//response_action 是dhcp服务器端向客户端发送的信息，此信息中已包含offer ip,故不能用于dhcp协商阶段
//ipv4_addr_match 匹配由dhcp客户端发出来的到dhcp server的报文（未限制udp端口等信息）
static bool
                    ...
                    struct ds *ipv4_addr_match)
{
    ...
    if (error || ((offer_ip ^ host_ip) & mask)) {
        ...
        *  */
    	//offer_ip与host_ip应仅有主机位不相等
        free(error);
        return false;
    }

    const char *server_ip = smap_get(
        &op->nbsp->dhcpv4_options->options, "server_id");//dhcp服务器地址
    ...
    smap_add(&dhcpv4_options, "netmask", netmask);//添加掩码信息
    free(netmask);
...
build_pre_acls(struct ovn_datapath *od, struct hmap *lflows)
{
     ...
     * allowed by default. */
    //为ingress 与egress的pre-acl表下发两条优先级为0的两条规则（其中match为1，action为next;)
    ...
    //如果存在有状态的acl，则所有报文需要经过连接跟踪action
    if (has_stateful) {
 ...
 * 'ip_address'. */
//自key中提取ip及port,将ip信息存放在ip_address中，将port存放在port中
static void
ip_address_and_port_from_lb_key(const char *key, char **ip_address,
                                uint16_t *port)
{
    ...
    *port = 0;
    ...
    ip_str = strsep(&next, ":");//取出next中的cidr地址
    if (!ip_str || !ip_str[0]) {
        static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
    ...
    ovs_be32 ip, mask;
    char *error = ip_parse_masked(ip_str, &ip, &mask);//将cidr地址转为ip及掩码形式
    if (error || mask != OVS_BE32_MAX) {
        static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
        VLOG_WARN_RL(&rl, "bad ip address for load balancer key %s", key);
        free(start);
...
//交换机规则构建
//1。报文合法性检查
//2.启用端口安全的接口，对其在出入两个方向进行mac,ip的限制
//3.对于已知端口mac地址及ip的，注入规则响应arp请求
//4.对于有dhcp选项的端口，为其注入dhcp报文响应逻辑
//5.如果配置有dns规则，则注入dns 透明代理规则，进行配置dns的拦截
//6.对于已知mac的端口，为其注入静态fdb表（对于存在unkown的按unkown group进行处理，对于出接口为flood的，按flood group处理）
static void
                    ...
                    struct hmap *lflows, struct hmap *mcgroups)
{
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        //如果此端口启用port_security配置，则仅容许安全组要求的源mac地址通过
        ds_clear(&match);
        ds_clear(&actions);
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        //localnet是与物理接口相连的，vtep是与物理交换机相连的
        //自这些口进来的报文默认放通，不回复
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        //注：localport在ovn中有两种，一种是提供dhcp服务，一种是为了提供metadata服务
        //注：router口就是对端与路由器相连的口
        //即对自已引入的三层口进行响应
        //在交换机上，如果当前口类型是router,localport且此口是up的，则进行arp响应
        //收到对这个ip地址的请求，进行响应

        for (size_t i = 0; i < op->n_lsp_addrs; i++) {
            for (size_t j = 0; j < op->lsp_addrs[i].n_ipv4_addrs; j++) {
                ds_clear(&match);
                ds_put_format(&match, "arp.tpa == %s && arp.op == 1",//收到对arp.tpa的arp请求
                ...
                ds_put_format(&actions,
                    ...
                    "arp.tpa = arp.spa; "
                    "arp.spa = %s; "
                    "outport = inport; " //指定出接口即为入接口
                    "flags.loopback = 1; "
                    "output;",
                    op->lsp_addrs[i].ea_s, op->lsp_addrs[i].ea_s,
                    op->lsp_addrs[i].ipv4_addrs[j].addr_s);
                ovn_lflow_add(lflows, op->od, S_SWITCH_IN_ARP_ND_RSP, 50,
                              ds_cstr(&match), ds_cstr(&actions));//添加arp响应报文回复规则
                 ...
                 * intent.) */
                //如果报文的入接口本来就具有这样ip地址，理论上说，它自已就知道自已有这个ip，但仍然
                //自这个接口扔出arp请求，此时需要放通，为什么？（支持arp冲突检测；当然，如果仅考虑这里的
                //路由器是模拟的，我也觉的意义不大，但也的确可以用来刷物理交换机的fdb表项
                ds_put_format(&match, " && inport == %s", op->json_key);
                ovn_lflow_add(lflows, op->od, S_SWITCH_IN_ARP_ND_RSP, 100,
                              ds_cstr(&match), "next;");
            }
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        if (!lsp_is_enabled(op->nbsp) || !strcmp(op->nbsp->type, "router")) {
        	...
        	//对端是router,按我们的默认，不会处理dhcp报文，故不开启
            continue;
        }
        ...
        for (size_t i = 0; i < op->n_lsp_addrs; i++) {
            for (size_t j = 0; j < op->lsp_addrs[i].n_ipv4_addrs; j++) {
                ...
                if (build_dhcpv4_action(
                	...
                	//确认是对应的接口发送的，dhcp 报文后(discover)，执行相应动作
                	//支持dhcp discover报文
                    struct ds match = DS_EMPTY_INITIALIZER;
                    ds_put_format(
                     ...
                     */
                    //确认自dhcp客户端发出的报文，执行options_action
                    //支持dhcprequest报文
                    ds_put_format(
                        &match, "inport == %s && eth.src == %s && "
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        if (lsp_is_enabled(op->nbsp)) {
            ovn_multicast_add(mcgroups, &mc_flood, op);//在flood组播组中加入此接口
        }
    }
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        ...
        for (size_t i = 0; i < op->nbsp->n_addresses; i++) {
            ...
            if (ovs_scan(op->nbsp->addresses[i],
            	...
            	//port无明确mac地址，加入到fdb失败组播组（设置为unknown的，比如系统外部的设备）
                if (lsp_is_enabled(op->nbsp)) {
                    ovn_multicast_add(mcgroups, &mc_unknown, op);
                    op->od->has_unknown = true;
                }
            } else if (is_dynamic_lsp_address(op->nbsp->addresses[i])) {
            	//port为动态，但mac地址是静态的，走静态fdb表逻辑
                if (!op->nbsp->dynamic_addresses
            	...
            	//如果此接口对端为路由器，则取对端路由器的mac地址，并走静态fdb表逻辑
                ...
                ds_put_format(&actions, "outport = %s; output;", op->json_key);
                ovn_lflow_add(lflows, op->od, S_SWITCH_IN_L2_LKUP, 50,
                              ds_cstr(&match), ds_cstr(&actions));//对端为普通路由器接口
    ...
    HMAP_FOR_EACH (od, key_node, datapaths) {
        ...
        if (od->has_unknown) {
        	//datapath中存在mac unknow的port,使其在unknow范围内flood
     ...
     *                 (priority 100). */
    //入接口采用的策略是默认丢弃，出接口采用的策略时默认放通,出接口策略默认广播组播可通
    HMAP_FOR_EACH (od, key_node, datapaths) {
        if (!od->nbs) {
            continue;
        }
    ...
    if (is_ipv4) {
        ...
        for (int i = 0; i < op->lrp_networks.n_ipv4_addrs; i++) {
            ...
            if (!((na->network ^ ip) & na->mask)) {
                 ...
                 * overlap. */
            	//依据上面的描述（接口间网段不会重叠的问题）只要检查出来一个，即返回
                return na->addr_s;
            }
        }
    } else {
...
//添加路由表(支持src-ip方式进行路由，默认采用dst方式路由）
static void
          ...
          const char *gateway, const char *policy)
{
    ...
    //举个例子，形如：ip4.src == 1.1.1.1/24
    ds_put_format(&match, "ip%s.%s == %s/%d", is_ipv4 ? "4" : "6", dir,
                  network_s, plen);

    //ttl -1 ,然后将下一跳地址写入到0号寄存器中，有网关填网关地址，非网关填目的ip地址
    struct ds actions = DS_EMPTY_INITIALIZER;
    ...
    //设置寄存器1为出接口ip地址（为什么需要？发arp时需要??），设置源mac地址为本接口mac
    //设置出接口为本接口
    ds_put_format(&actions, "; "
                  "%sreg1 = %s; "
                  "eth.src = %s; "
                        ...
                        const struct nbrec_logical_router_static_route *route)
{
    ...
    char *error = ip_parse_cidr(route->nexthop, &nexthop, &plen);//取下一跳
    if (!error) {
        if (plen != 32) {//下一跳必须是一个主机地址
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
    ...
    if (is_ipv4) {
        ...
        error = ip_parse_cidr(route->ip_prefix, &prefix, &plen);//校验网段设置
        if (error) {
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
    ...
    if (route->output_port) {
        ...
        //指定了出接口，还指定了下一跳，故这里比较扯，还好作者认为如果下一跳在出接口上找不到同网段的，则认为没有指定
        //如果指定了又找到同网段的，这实际上是重复信息，但两者都对，所以也没有问题
    	...
    	//仅指定了下一跳，遍历所有接口确认下一跳和那个接口处于同一网段
        int i;
                    ...
                    struct hmap *lflows)
{
    ...
    //实现三层接口对收包校验（ip层面检查）
    //1.不支持三层组播路由;2.不合法的源ip地址，不合法的全0目的ip地址
    //3.除arp,nd报文外，丢掉广播报文;4.超时的ttl
    //5.容许其它报文通过
    HMAP_FOR_EACH (od, key_node, datapaths) {
        if (!od->nbr) {
            continue;
         ...
         * (priority 100). */
        //组播报文被丢弃，错误的src ip被丢弃，目的ip为0.0.0.0/8的被丢弃（dhcp不在路由器里做）
         ...
         * not be forwarded.*/
        //eth.bcast指代的是目的mac地址为全1地址（丢掉广播地址）
        ...
        //如果收到ttl为0或者为1的报文，则丢弃（注：没有发送icmp报文）
        ds_clear(&match);
    ...
    //三层简单协议处理：
    //1.丢弃掉自外面发送的源ip为本接口ip的报文（安全问题及防ip简单环路）
    //2.响应对本接口某个ip地址的ping
    //3.响应对本接口某个ip地址的arp请求
    //4.响应对本接口代理的某个vip地址的arp请求
    //5.响应对本接口代理的arp的请求（1。报文出去时做snat;2.
    //5.丢弃掉其它会去往本机的报文（路由器是模拟的无法处理这些流量，故丢弃）
    HMAP_FOR_EACH (op, key_node, ports) {
        if (!op->nbrp) {
            continue;
        }
        ...
        SSET_FOR_EACH(ip_address, &all_ips) {
            ...
            if (!ip_parse(ip_address, &ip) || !ip) {
                continue;//未成功解析ip地址,忽略
            }

            ds_clear(&match);
            //收到vips的arp请求，回应arp响应
            ds_put_format(&match,
        ...
        for (int i = 0; i < op->od->nbr->n_nat; i++) {
            ...
            ovs_be32 ip;
            if (!ip_parse(nat->external_ip, &ip) || !ip) {//取外部ip
                static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
            ...
            if (!strcmp(nat->type, "snat")) {//只记录做snat的ip
             ...
             * handling. */
            //报文在出去时会被做了snat,现在对端发arp请求做了snat的ip地址的mac地址，需要对此进行回应
            ds_clear(&match);
            ds_put_format(&match,
                          "inport == %s && arp.tpa == "IP_FMT" && arp.op == 1",
                          op->json_key, IP_ARGS(ip));
            ...
            if (op->od->l3dgw_port && op == op->od->l3dgw_port) {
            	//对于l3dgw_port需要代理他人arp,故需要将mac改为它人的
                struct eth_addr mac;
                if (nat->external_mac &&
                    eth_addr_from_string(nat->external_mac, &mac)
                    && nat->logical_port) {
        ...
        //这个路由器是模拟的，故无法处理到本机的报文（snat ip地址除外），故这些报文需要丢弃
        ds_clear(&match);
        ds_put_cstr(&match, "ip4.dst == {");
    ...
    HMAP_FOR_EACH (od, key_node, datapaths) {
        ...
        const char *dnat_force_snat_ip = get_force_snat_ip(od, "dnat",
                                                           &snat_ip);//取dnat_force_snat_ip
        const char *lb_force_snat_ip = get_force_snat_ip(od, "lb",
                                                         &snat_ip);//取lb_force_snat_ip
        ...
        for (int i = 0; i < od->nbr->n_nat; i++) {
            ...
            char *error = ip_parse_masked(nat->external_ip, &ip, &mask);//取转换后ip
            if (error || mask != OVS_BE32_MAX) {
            	//静态ip不能是网段
                static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 1);
            ...
            error = ip_parse_masked(nat->logical_ip, &ip, &mask);//取转换前ip
             ...
             *
             * 之前，我们对这个报文进行过snat,故对端在响应时，目的ip地址填充的是snat之后的地址
             * 实际上这个时候就是需要做dnat.（作者起的名字太恶心，undo snat,呵呵。）
             *
             ...
             * egress pipeline.
             *
             *  说白了，作者就是要在dnat处理之前对这种做过snat的做个特殊处理。（为什么？
             *  	因为这种dnat是由snat动态生成的，不是由dnat规则来确定的。严格说，它不能用规则
             *  	进行转换，当前一般它也命中不了。但是他要命中了呢？ //这种情况可以发生在，上层添加了dnat规则
             *  	将路由器上的ip地址作了端口映射，而在此之前有一条流通过snat分配出去了这个端口，故配置后
             *  	原有的流没断开前仍占用此端口，端口映射仅对新建的流生效（这种情况下没有bug吗？）
             *  ）
             **/
            if (!strcmp(nat->type, "snat")
                ...
                if (!od->l3dgw_port) {
                    ...
                    ovn_lflow_add(lflows, od, S_ROUTER_IN_UNSNAT, 100,
                                  ds_cstr(&match), "ct_snat;");//l3dgw_port收到按snat反向做dnat
                    ...
                    ovn_lflow_add(lflows, od, S_ROUTER_IN_UNSNAT, 50,
                                  ds_cstr(&match),
                                  REGBIT_NAT_REDIRECT" = 1; next;");//从其它口收到的，中转给l3dgw_port做nat处理
                }
            }
            ...
            //依据external_ip实现对是否做dnat的检查，然后将external_ip做dnat转换为logical_ip
            if (!strcmp(nat->type, "dnat")
                ...
                if (!od->l3dgw_port) {
                    ...
                    ovn_lflow_add(lflows, od, S_ROUTER_IN_DNAT, 50,
                                  ds_cstr(&match),
                                  REGBIT_NAT_REDIRECT" = 1; next;");//dnat转l3dgw_port处理
                }
            }
            ...
            if (od->l3dgw_port && (!strcmp(nat->type, "dnat")
                ...
                ds_put_format(&actions, "ct_dnat;");//按连接跟踪的dnat进行反向处理
            ...
            if (!strcmp(nat->type, "snat")
                ...
                if (!od->l3dgw_port) {
                    ...
                    ds_put_format(&match, "ip && ip4.src == %s",
                                  nat->logical_ip);
                    ds_clear(&actions);
                    ds_put_format(&actions, "ct_snat(%s);", nat->external_ip);//按规则进行snat处理
    ...
    HMAP_FOR_EACH (op, key_node, ports) {
        if (op->nbrp) {
            ...
            if (op->peer && op->nbrp->peer) {
                if (op->lrp_networks.n_ipv4_addrs) {
                    ds_clear(&match);
                    //出接口是对端的口，且ip地址是本端的ip地址，则目的mac地址改为本端mac
                    //当前设备发包出去，更改为自已的src-mac
    ...
    HMAP_FOR_EACH (od, key_node, datapaths) {
        ...
        if (od->l3dgw_port && od->l3redirect_port) {
        	...
        	//出接口为l3dgw_port，则将出接口更改为l3redirect_port
            ds_clear(&match);
             ...
             */
            //如果目的mac未知，则这些流也转给l3redirect_port去处理（这种是集中转发）
             ...
             struct hmap *ports)
{
    ...
    build_lswitch_flows(datapaths, ports, &lflows, &mcgroups);//构造交换机的flow
    build_lrouter_flows(datapaths, ports, &lflows);//构造路由器的flow
    ...
    SBREC_LOGICAL_FLOW_FOR_EACH_SAFE (sbflow, next_sbflow, ctx->ovnsb_idl) {
        ...
        if (lflow) {
        	//系统中已存在此flow,不需要加入，故自flows表中删除
            ovn_lflow_destroy(&lflows, lflow);
        } else {
        	//当前lflows表中已不能再计算出此流表了，故sb库中移除
            sbrec_logical_flow_delete(sbflow);
        }
    }

    //需要新增的flow存放在lflows表中
    ...
    HMAP_FOR_EACH_SAFE (lflow, next_lflow, hmap_node, &lflows) {
        ...
        sbrec_logical_flow_set_external_ids(sbflow, &ids);//流的调试信息
        smap_destroy(&ids);
    ...
    //当前组播组实际上定义了两个广播域：一是flood域，一个是fdb表查询missing在未知的接口间查询（优化）
    ...
    SBREC_MULTICAST_GROUP_FOR_EACH_SAFE (sbmc, next_sbmc, ctx->ovnsb_idl) {
        ...
        struct ovn_multicast *mc = ovn_multicast_find(&mcgroups, od, &group);
        if (mc) {
        	//mcgroups中已存在，故mcgroups中移除，sb数据库中更新
             ...
             struct ovsdb_idl_loop *sb_loop)
{
    ...
    build_datapaths(ctx, &datapaths);//同步datapath（增加新的dp,删除无用的dp)
    build_ports(ctx, &datapaths, chassis_index, &ports);//同步port信息(注：删除的port地址什么时候回收）
    build_ipam(&datapaths, &ports);//为要求动态地址的port申请ip
    build_lflows(ctx, &datapaths, &ports);//同步lflows，组播组表

    sync_address_sets(ctx);//同步address_sets表
    sync_dns_entries(ctx, &datapaths);//同步dns表项

    //开始销毁不再需要的datapaths变量，ports变量
...
update_logical_port_status(struct northd_context *ctx)
{
    ...
    SBREC_PORT_BINDING_FOR_EACH(sb, ctx->ovnsb_idl) {
        ...
        if (sb->chassis && (!nbsp->up || !*nbsp->up)) {//如果接口绑定chassis，则nb更新为up
            bool up = true;
            nbrec_logical_switch_port_set_up(nbsp, &up, 1);
        } else if (!sb->chassis && (!nbsp->up || *nbsp->up)) {//否则更新为down
            bool up = false;
            nbrec_logical_switch_port_set_up(nbsp, &up, 1);
        }
    }
...
check_and_add_supported_dhcp_opts_to_sb_db(struct northd_context *ctx)
{
    ...
    SBREC_DHCP_OPTIONS_FOR_EACH_SAFE(opt_row, opt_row_next, ctx->ovnsb_idl) {
        ...
        if (dhcp_opt) {
        	//不需要添加，sb库中已存在，故自dhcp_opts_to_add中删除
...
main(int argc, char *argv[])
{
    ...
    while (!exiting) {
        ...
        chassis_index_init(&chassis_index, ctx.ovnsb_idl);//收集南库的chassis

        //将north库的内容添加到sb库中
        ovnnb_db_run(&ctx, &chassis_index, &ovnsb_idl_loop);//northd最重要的任务
        ovnsb_db_run(&ctx, &ovnsb_idl_loop);
        if (ctx.ovnsb_txn) {
        	//在sb库中构造并添加我们支持的dhcp选项
